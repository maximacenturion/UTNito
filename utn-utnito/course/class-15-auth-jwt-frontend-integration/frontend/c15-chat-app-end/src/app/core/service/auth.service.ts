...
export class AuthService {
  ...
  refreshAccessToken(): Observable<boolean> {
    // Refresh flow uses refresh token to replace access token only.
  ...
  checkAccessTokenExpiration(accessToken: string | null): Observable<string | null> {
    // No token available, request will continue as unauthenticated.
    if (!accessToken) {
      return of(null);
    }

    // Token still valid, reuse it directly.
    if (!this.isTokenExpired(accessToken)) {
      return of(accessToken);
    }

    // Token expired, try refresh before sending the protected request.
