...
export class AuthController extends AbstractController {
  ...
  /** Refreshes access token from a valid refresh token. */
  @Post('refresh-token')
  // Swagger decorator: describes endpoint behavior in Swagger docs.
  ...
  refreshToken(@Body() request: RefreshTokenRequest) {
    const refreshedToken = this.authService.refreshToken(request);
    return this.createOkResponseWithMessage(refreshedToken, 'Access token refreshed');
  }
